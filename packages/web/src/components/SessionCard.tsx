...
function getAlerts(session: DashboardSession): Alert[] {
  ...
  const greptileBlocker = pr.mergeability.blockers.find(
    (blocker) => blocker.startsWith("Greptile ") || blocker.startsWith("Awaiting Greptile"),
  );
  if (greptileBlocker) {
    alerts.push({
      key: "greptile",
      label: greptileBlocker,
      className:
        "border-[rgba(245,158,11,0.3)] bg-[rgba(245,158,11,0.08)] text-[var(--color-status-attention)]",
      url: pr.url,
      actionLabel: "ask to review",
      actionMessage: `Please address the Greptile review blockers on ${pr.url} and push updates.`,
    });
  }
