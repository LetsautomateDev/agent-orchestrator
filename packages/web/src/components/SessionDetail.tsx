...
function IssuesList({ pr }: { pr: DashboardPR }) {
  ...
  for (const blocker of pr.mergeability.blockers) {
    if (blocker.startsWith("Greptile ") || blocker.startsWith("Awaiting Greptile")) {
      issues.push({ icon: "●", color: "var(--color-status-attention)", text: blocker });
    }
  }
